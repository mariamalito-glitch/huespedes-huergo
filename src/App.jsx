...
function HuespedCard({ h, index, total, showId = false }) {
  ...
  return (
    <div style={{ background: bg, borderRadius: 10, padding: "10px 14px", marginBottom: index < total - 1 ? 8 : 0 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.65)", marginTop: 3 }}>
            {showId && h.id && (
              <span style={{ color: "rgba(255,255,255,0.95)", fontWeight: 700, background: "rgba(255,255,255,0.15)", padding: "1px 7px", borderRadius: 5, marginRight: 6 }}>
                DNI/ID: {h.id}
              </span>
            )}
        ...
        <div style={{ textAlign: "right", whiteSpace: "nowrap" }}>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.55)" }}>Estadía</div>
      ...
      <div style={{ background: C.navy, padding: "0 1.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 0 0" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {isAdmin && (
              <button onClick={() => { setUserRole("general"); setTab("deptos") }} style={{ background: "rgba(255,255,255,0.12)", border: "none", color: "#fff", fontSize: 12, padding: "6px 12px", borderRadius: 7, cursor: "pointer", fontWeight: 500 }}>Salir</button>
            )}
          </div>
        </div>
                ...
                {filteredDeptos.map(d => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {hList.length > 0 && (
                        <div>
                          {hList.map((h, idx) => <HuespedCard key={h.id} h={h} index={idx} total={hList.length} />)}
                        </div>
                      )}
                    </div>
                  )
            ...
            {hList.length > 0 ? (
              <>
                <div style={{ marginBottom: 16 }}>
                  {hList.map((h, idx) => <HuespedCard key={h.id} h={h} index={idx} total={hList.length} showId={true} />)}
                </div>
